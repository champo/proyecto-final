...
public class Mask3 extends DirectionalDetector {

	public Mask3(final Synthethizer synth, final boolean simple) {
		super(synth, simple);
	}

	@Override
	protected Mask buildSimpleMask(final int width, final int height) {
		return new Mask(width, height, new double[][] { { 1, 1, 1 }, { 0, 0, 0 }, { -1, -1, -1 } });
	}
	...
	protected BaseMask[] buildAllMasks(final int width, final int height) {
		return new BaseMask[] {
				 ...
				 * -1 -1 -1
				 */
			new Mask(width, height, new double[][] { { 1, 1, 1 }, { 0, 0, 0 }, { -1, -1, -1 } }),
			new Mask(width, height, new double[][] { { 0, 1, 0 }, { 1, 0, -1 }, { 0, -1, 0 } }),
			new Mask(width, height, new double[][] { { 1, 0, -1 }, { 1, 0, -1 }, { 1, 0, -1 } }),
			new Mask(width, height, new double[][] { { 0, -1, 0 }, { 1, 0, -1 }, { 0, 1, 0 } }),
		};
	}
