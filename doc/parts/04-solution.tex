...
de la imagen y detectar e ignorar sectores de la imagen que, con total certeza,
no correspondan a jugadores. A continuación se describen los métodos
evaluados.
...
\begin{figure}[H]
  \centering
    \begin{minipage}[t]{.45\textwidth}
      ...
      \caption{Un cuadro del video del partido entre Boca e Independiente.
...
del video genera muchos falsos negativos, sobre todo \textit{glitches}
alrededor de las líneas de la cancha, lo que les otorgaba a estos puntos un
mayor valor de energía del que realmente tenían. En la Figura
...
fondo, pero en el siguiente cuadro cambios en la imagen hacen que descarte de
manera errónea ciertas partes del fondo.
...
\begin{figure}[H]
  \centering
    ...
    \begin{minipage}[t]{.45\textwidth}
      ...
      \caption{Recorte del cuadro \#27 del video, con el fondo según el algoritmo en azul.
      \label{fig:tanos-fondo}}
    \end{minipage}
    \begin{minipage}[t]{.45\textwidth}
      \centering
      \includegraphics[width=\linewidth]{./images/tanos-fondo-f28.png}
      \caption{Recorte del cuadro \#28 del video, con el fondo según el algoritmo en azul.
...
Se encontró que las líneas blancas que delimitan la cancha y sus distintas partes
...
líneas son parte de un jugador, como en la figura \ref{fig:confusion-linea}.
...
Para la eliminación del césped del campo de juego se requiere caracterizarlo de
alguna forma. Para esto, se realiza un análisis de los colores de la imagen
recortada, es decir la imagen resultante luego aplicar la técnica de
\textit{recorte}, detallada en \ref{subsec:crop-tribunas}, una vez
seleccionados los contornos iniciales de los jugadores. Sobre este cuadro, se
calcula el valor promedio y desvío estándar de todo píxel que no forme parte de
los jugadores o un elemento ya conocido del fondo (por ejemplo, las líneas).
...
funciones fundamentales. Una es la función de probabilidad $p$ y la otra la
función característica $v$ (ver Sección \ref{sec:ac}). En esta Sección se
describe la elección de ambas funciones para este trabajo.
...
cada cuadro, siendo $\mathbf{v}$ un vector con las características de un contorno
dado, se calcula el promedio de los valores de las características para todos
los píxeles pertenecientes al contorno y se lo denomina $\hat{\mathbf{v}}$. A
continuación se actualiza $\mathbf{v}$ y se lo reemplaza por un nuevo valor
$\mathbf{v_n}$ calculado de acuerdo a:
...
%% TODO champoman esto no está muy bien redacatado. Intenté arreglarlo un poco
%% pero tampoco estoy muy convencido. Se aceptan aportes.
Dada las restricciones iniciales del problema, se puede aseverar que la imagen
de un dado jugador siempre va a estar contenida en un rectángulo, y calcular las
dimensiones máximas posibles de ese rectángulo. Se puede
entonces limitar el tamaño del contorno, reduciendo la posibilidad de que el
contorno intente seguir objetos de características parecidas al jugador, pero
cuyas dimensiones sean mayores al rectángulo más grande, señalando que el
objeto no es un jugador. En el caso de querer evaluar si un punto pertenece al
fondo o no, esta mejora no es deseable y por lo tanto este trabajo cuenta con
dos funciones, la sugerida anteriormente $f$ para el fondo y una $f_j$ para los
jugadores. Se definen de la siguiente manera:
...
Como explicado en la Sección \ref{sec:ac-problemas}, no resulta directo construir
una función $v$ para camisetas rayadas. Si bien el agregado del Desvío Estandard 
contribuye a mejorar el seguimiento en estos casos, no soluciona totalmente el
problema. Para poder seguir correctamente a equipos cuyas camisetas presentan este
desafio, optamos por ignorar sus caracteristicas y seguir por lo que no son. Es
decir, no son jugadores del otro equipo, no son cesped y no son lineas de la cancha.

Como es necesario saber donde estan los jugadores del primer equipo, se debe utilizar
Contornos Activos dos veces. Una primera vez haciendo un seguimiento por caracteristicas,
luego de la cual se aplica el procedimiento \textit{complemento}, detallado en el Algoritmo
\ref{alg:complemento}. La salida de este procedimiento marca todo punto que no fue
identificado bajo ningun conjunto con el color \textit{cyan}, y aplica un operador
morfologico de expansión sobre todos estos puntos.
...
Conociendo que puntos son candidatos a ser jugadores del segundo equipo, se hace una segunda 
ejecución de Contornos Activos donde todos los contornos tienen la caracteristica \textit{RGB}
\textit{cyan}.
