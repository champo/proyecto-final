...
\[
  P(x, y) = \exp \[ (- \vert ( x - \vert \tilde{x} + V_x \sign(\cos \theta) \vert )
  + ( y - \vert \tilde{y} + V_y \sign(\sin \theta) \vert)\vert ^ 2 / 2\theta^2) /\sigma \sqrt{2\pi}\]
\]
...
$x$ e $y$, $\theta$ es $\atan(\tfrac{V_y}{V_x}$, $R_p$ es el radio de la pelota
