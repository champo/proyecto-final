...
su utilidad y aplicación en la resolución del problema, así como su cálculo numérico.
...
%TODO Again, se entiende? (Seen by: Alvaneitor)
...
Se declara una función de probabilidad $p$ que permite saber que tan probable es que un píxel forme parte una dada región.
Para esto, es necesaria una función $v$ que dado un píxel, devuelve un vector de características $v(x)$.
...
Como ejemplo, se toman los valores \textit{RGB} como vector de características y
$p(v(x) \vert \Omega_i) = \| v(x) - v_i \| $, donde $v_i$ es el valor promedio \textit{RGB} de toda la región.
...
%TODO representa la competencia entre regiones? (Seen by: Alvaneitor)
...
\left\{
    \begin{array}{ll}
        3  & \mbox{if } x \in \Omega_{0} \mbox{  y  } x \notin L_{out} \\
        1  & \mbox{if } x \in L_{out}\\
        -1  & \mbox{if } x \in L_{in}\\
        -3 & \mbox{if } x \in \Omega_{1} \mbox{  y  } x \notin L_{in} \\
    \end{array}
\right.
\end{equation}
...
a la definición formal dada anteriormente.
%TODO "definición formal dada anteriormente" cual? maybe referencia? (Seen by: Alvaneitor)
...
% TODO (eordano): Ref correcta a los tanos
...
\begin{equation}
  \label{eq:tanos-bf1}
  % TODO (eordano): Copiar el latex de los tanos
\end{equation}
...
% TODO: El paper nombra que utiliza un análisis de conectividad de puntos de foreground y la mofología de los conjuntos obtenidos
% pero no explica un carajo como lo hace.
...
%TODO mayber footnote o referencia a que es una distancia de Mahalanobis? (Seen by:alvaneitor)
...
sector candidato de la imagen a contener a la pelota o a una persona en el campo de
...
del césped de estadios de Europa. Esta característica no se ve en las canchas
...
\begin{itemize}
  ...
  \item \textbf{Detección de \textit{blobs}}: Otros objetos que no sean verdes
    ...
    segmentos definidos, diferenciados por color, correspondientes al
...
\begin{itemize}
  \item \textbf{Seguimiento de personas}: No se da el caso de que un jugador
...
Una vez detectada la pelota, se utiliza seguimiento hasta que no se confia en el resultado del algoritmo de seguimiento.
