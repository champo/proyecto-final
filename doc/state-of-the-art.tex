...
una o varias cámaras de video. Se presentan las bases teoricas para estos
métodos y se analizan características y limitaciones de cada uno, asi como la
factibilidad de conocer el estado del juego completo en cada momento.
} \end{customabstract}

\part*{Introducción}

Dentro del área de análisis de imágenes, uno de los problemas más relevantes y
estudiados es el problema de reconocer un objeto en una secuencia de
...
En la secciónes \ref{sec:6-camaras}, \ref{sec:8-camaras}, \ref{sec:var-camaras} y \ref{sec:tv-video} 
se discuten los avances modernos en esta rama. 
...
Se dice que el predictor es lineal al obligarse a $\beta_i \in \mathbb{R}$ 
...
donde cada fila representa una función de predicción.  De dos
cuadros consecutivos se obtiene el vector de cambios en el valor de píxeles $X =
...
Una vez conocida la posición, se toman $n$ muestras aleatorias del valor de píxeles en ese cuadro.

Luego se crean $m$ perturbaciones de la posición del objeto, y se computa la homografia $H_i$ que representa esa perturbación.
...
\end{equation}
Que es un sistema de ecuaciones lineales que puede ser resuelto de manera sencilla.
...
\citeauthor*{alp} (\cite{alp}) introducen una forma para manejo de oclusiones parciales de manera eficiente.
...
\begin{eqnarray*}
    ...
    A &=& Y H^T(HH^T)^{-1}
\end{eqnarray*}
Esto permite mediante una fórmula compleja pero eficiente actualizar $A$ rápidamente para remover y agregar \textit{templates}.
O sea, permite ignorar secciones ocultas del objeto hasta que éstas vuelven a ser visibles.
...
del objeto adquiere gran importancia.
...
asigna una importancia relativa a la distancia de cada componente de $d_{ez}$.
