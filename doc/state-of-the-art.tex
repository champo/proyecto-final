...
% TODO Poner referencia o footnote explica PCA, Principal Component Analisis
...
% TODO Poner referencia o explicar particle filter
...
disjunto de árboles de camino óptimo,
\footnote{Un árbol de camino óptimo es simplemente un árbol, en el contexto de grafos, el cual se obtiene a partir de un grafo subyacente, y que contiene todos
los nodos del grafo subyacente, y los caminos entre ellos y el nodo raíz del árbol que tienen costo mínimo.}
 con cada árbol con uno de los marcadores como raíz. La proyección del objeto es entonces tomada como la union de los árboles cuyas raices 
son marcadores internos.\\

\subsubsection{Algoritmo IFTrace}
 \begin{figure}[H]
...
IFTrace toma como input un video $I$, modelado como un array 3D de pixeles, siendo las dimensiones el alto y ancho de la imagen, y número de cuadro de la secuencia de video, y una 
...
Para cada cuadro de la secuencia, el algoritmo elije dos conjuntos de puntos o pixeles: los que pertenecen a la máscara del objeto $R_{o}$, y los que pertenecen a 
sus alrededores $R_{x}$. Luego se procede 
a localizarlos en el siguiente cuadro a través del procedimiento \textit{trackMarkers}. 

Si el conjunto de pixeles que representan al objeto $S_{0}^{(t)}$ no resulta vacío para este 
nuevo cuadro, entonces el seguimiento
fue exitoso, y se procede a utilizar IFT para, a partir de los dos conjuntos de pixeles $S_{0}^{(t)}$ y $S_{x}^{(t)}$, el clasificador de color del cuadro anterior $C^{(t-1)}$ 
...
Por otro lado, si el seguimiento del objeto falla para el nuevo cuadro, es decir si el conjunto de pixeles resultantes luego de aplicar \textit{trackMarkers} $S_{0}^{(t)}$ 
...
en algoritmo 1 en la figura \ref{fig:IFTrace-algorithm1}.
...
\footnote{ }
% TODO Poner footnote de (\textit{morphological erosion})
...
\footnote{ }
%TODO POner footnote con dilatacion morfolofica
 con cierto radio $\delta_{x}$, y tomando los pixeles a lo largo del borde del resultado.
...
Antes de comenzar, se inicializan los mapas para caminos triviales. Se agregan 
...
En comparación con la IFT, \textit{Corte de Grafos} tiene 2 grandes desventajas: es 
